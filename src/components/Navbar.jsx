              ...
              {pathnames.map((value, index) => {
                ...
                return (
                  <React.Fragment key={to}>
                    ...
                    <li className={`inline-flex items-center ${last ? "" : "text-[#9A9A9A]"}`}>
                      {last ? <span className="text-[#9A9A9A]">{formatSegment(value)}</span> : <div className="text-[#9A9A9A] ">{formatSegment(value)}</div>}
                    </li>
                  </React.Fragment>
                );
              })}
